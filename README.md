...
### @version 2.0
	1、
		...
		b) 默认的URL拆分是取URL中path部分的最后两个路径，如“http://localhost:3000/sasas/fasasdf/user/index”取user做控制器，
	...
	3、 日志记录模块的顺序是：mongodb > 文件日志 > 终端。  即先将日志记录到mongodb库里；若操作失败，再记录到日志文件里；若也操作失败，直接在终端打印出来。
